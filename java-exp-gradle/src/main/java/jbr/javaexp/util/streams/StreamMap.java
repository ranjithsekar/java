...
public class StreamMap {
    ...
    public static void countTotalPrice() {
        List<Fruit> fruitsObj = Arrays.asList(new Fruit("Apple", 100), new Fruit("Banana", 200),
                new Fruit("Grapes", 300), new Fruit("Orange", 400));
        System.out.println(fruitsObj.stream().map(e -> e.getPrice()).collect(Collectors.summingInt(Integer::intValue)));
    }
    ...
    public static void streamReverse() {
        ...
        String namesR = fruitsObj.stream().sorted(Comparator.comparing(Fruit::hashCode).reversed())
